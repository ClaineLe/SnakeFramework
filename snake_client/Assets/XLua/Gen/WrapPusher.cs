...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<EScreenPriority>(translator.PushEScreenPriority, translator.Get, translator.UpdateEScreenPriority);
        ...
        int EScreenPriority_TypeID = -1;
		int EScreenPriority_EnumRef = -1;
        
        public void PushEScreenPriority(RealStatePtr L, EScreenPriority val)
        {
            if (EScreenPriority_TypeID == -1)
            {
			    bool is_first;
                EScreenPriority_TypeID = getTypeId(L, typeof(EScreenPriority), out is_first);
				
				if (EScreenPriority_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(EScreenPriority));
				    EScreenPriority_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, EScreenPriority_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, EScreenPriority_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for EScreenPriority ,value="+val);
            }
			
			LuaAPI.lua_getref(L, EScreenPriority_EnumRef);
			LuaAPI.lua_pushvalue(L, -2);
			LuaAPI.xlua_rawseti(L, -2, (int)val);
			LuaAPI.lua_pop(L, 1);
        ...
        public void Get(RealStatePtr L, int index, out EScreenPriority val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != EScreenPriority_TypeID)
				{
				    throw new Exception("invalid userdata for EScreenPriority");
				}
                ...
                if (!CopyByValue.UnPack(buff, 0, out e))
                {
                    throw new Exception("unpack fail for EScreenPriority");
                }
				val = (EScreenPriority)e;
                
            }
            else
            {
                val = (EScreenPriority)objectCasters.GetCaster(typeof(EScreenPriority))(L, index, null);
            }
        }
		
        public void UpdateEScreenPriority(RealStatePtr L, int index, EScreenPriority val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for EScreenPriority ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(EScreenPriority[]))
			{
			    EScreenPriority[] array = obj as EScreenPriority[];
				translator.PushEScreenPriority(L, array[index]);
				return true;
			}
            return false;
		}
